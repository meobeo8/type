...
function lib.handleAlert(index, alertFrame, isRemove, displayTime)
    if alertFrame.Parent == coregui.FE2Notifs[';3'] then
        local totalIndex = (index > 4 or isRemove) and index or index - 1
        local direction = (index > 4 or isRemove) and "In" or "Out"

        if index > 4 or isRemove then
            alertFrame.ZIndex = 0
            table.remove(table1, index)

            local tweenOut = TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
            TweenService:Create(alertFrame.Alert_Shadow, tweenOut, { ImageTransparency = 1 }):Play()
            TweenService:Create(alertFrame.Alert, tweenOut, {
        ...
        alertFrame:TweenPosition(
            UDim2.new(0.5, 0, 0.03 + ((index - 1) * NOTIFY_GAP), 0),
            direction, "Sine", 0.325, true
        )
    end
end
